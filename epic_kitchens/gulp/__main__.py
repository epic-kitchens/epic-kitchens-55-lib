...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "out_folder", type=str, help="Directory to store the gulped files."
    )
    parser.add_argument(
        "labels_pkl",
        type=str,
    ...
    if args.modality.lower() == "flow":
        epic_adapter = adapter.EpicFlowDatasetAdapter(
            args.in_folder, labels, args.frame_size, args.extension
        )
    ...
    ingestor = GulpIngestor(
        epic_adapter, args.out_folder, args.segments_per_chunk, args.num_workers
    )
    ingestor()
